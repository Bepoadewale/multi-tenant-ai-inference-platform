...
Quantization reduces memory and can improve fit/throughput but changes quality/performance trade-offs. Tensor parallelism splits a model across GPUs; data parallelism adds replicas. High GPU utilization is not automatically good user latency—queueing and tail TTFT matter. Local mock measurements must never be compared to GPU vLLM results.
